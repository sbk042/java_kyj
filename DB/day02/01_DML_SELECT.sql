/*
<select문 기본>
 => select문 속성명, ..., 속성명n from 테이블명;
 - 테이블에서 속성명1, ..., 속성명n의 값들을 조회한다는 뜻이다.
 - 전체를 조회하려면 *을 이용한다.
 
 <as >
 => 속성 또는 테이블명을 일시적으로 변경해서 사용할 때 쓰는 키워드
 속성명1 as 속성명2 => 속성명1을 일시적으로 속성명2로 사용
*/
use course; -- 데이터 베이스를 선택

-- 등록된 모든 학생의 모든 정보를 조회하는 쿼리를 작성하겠음
select * from student;
-- 등록된 모든 학생의 학번을 조회하는 쿼리를 작성하겠음
...
like: 와일드카드와 함께 사용되며, 값이 주어진 와이드가 포함된 문자열과 
	  같은지를 비교할 때 사용된다.
_   : 한글자를 의미하는 와일드 카드
%   : 0글자이상을 의미하는 와일드 카드
...
select * from student where name like '___'; -- _가 3개 => 3자
-- 성이 유씨인 학생 정보를 조회하는 쿼리
select * from student where name like '유__'; -- 이렇게 쓰면 이름이 두 글자인 유씨를 못 찾음
...
-- 방송학과 코드는 135이다.
-- 방송학과 학생을 조회하는 쿼리
select * from student where major like '방송학과';
...
<정렬방법>
	- ASC  : 오름차순. 기본값. 행이 아래로 갈수록 값이 커지면 오름차순
    - DESC : 내림차순. 행이 아래로 갈수록 값이 작아지면 내림차순
*/
/*
<LIMIT>
 = 행(튜플)을 원하는 위치부터 원하는 개수를 가져오기 위한 방법
...
/*
<GROUP BY절>
 => 같은 값을 가지는 행들을 묶어서 평균을 내거나 개수를 세는 동의 작업을 할 때 사용
<HAVING절>
 => 집계함수에 조건을 걸 때 사용한다.
...
-- 글쓰기와 영어가 아닌 모든 과목의 수강 정보를 조회하는 쿼리
select * from course 
	where subject_code != all(select code from subject where title in('글쓰기','영어'));
select * from course 
	where subject_code in(select code from subject where title !='글쓰기' && title != '영어');

-- 학생 정보를 이름순으로 오름차순 정렬하여 조회
select * from student order by name asc; -- asc를 생략가능
-- 학생 정보를 이름순으로 내림차순 정렬하여 조회
select * from student order by name desc;
-- 학생 정보를 전공, 이름순으로 오름차순으로 정렬하여 조회
select * from student order by major asc, name asc; -- 가나다라 순으로 정렬된다., asc생략가능
-- 학생들이 소속된 과를 조회하는 쿼리
select distinct major from student; -- distinct : 중복없애주기

-- 학생들 정보를 학번순으로 오름차순 정렬 후 위에서 2명의 학생 정보를 조회하는 쿼리
select * from student order by num asc limit 2;
-- 학생들 정보를 학번순으로 오름차순 정렬 후 3번째 4번째 학생 정보를 조회
...
SET GLOBAL sql_mode=(SELECT REPLACE(@@squl_mode,'ONLY_FULL_GROUP_BY',''));
-- 각 과별 학생수를 조회
select major as 학과, count(num) as 학생수 from sutdent group by major;
...
group by 
	subject_code
having
	수강생수 >= 4; -- 수강생수를 count(*)라고 작성해도 괜찮음
