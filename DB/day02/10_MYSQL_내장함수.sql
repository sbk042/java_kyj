...
ifnull(속성, 값) : 속성 값이 null이 아니면 속성값을 이용하고, 
				  null이면 값을 이용
...
instr(기준문자열, 찾을문자열);
 => 기준문자열에서 찾을 문자열이 몇번째에 있는지 알려주는 함수,
	없으면 0. 1부터 시작
...
: 문자열에 채울문자열을 왼쪽/오른쪽에 채워서 숫자개수만큼 길이가 되도록 한다.

replace(문자열, 원래 문자열, 바꿀문자열);
: 문자열에서 원래문자열을 찾아 있으면 바꿀 문자열로 수정한다.

substring(문자열, 시작위치, 길이);
: 문자열에서 시작위치부터 길이만큼 찾아서 반환한다.
  길이가 생략되면 시작위치부터 끝가지 반환
...
adddate(날짜, 차이)/subdate(날짜, 차이);
 : 날짜로부터 차이만큼 더한/뺀 날짜
 차이 : interval 숫자 단위(day, month, year)
 ...
 datediff(날짜1, 날짜2);
 : 날짜1과 날짜2 사이의 차이 => 일 기준으로 반환
 timediff(날짜1, 날짜2);
 : 날짜1과 날짜2 사이의 차이를 시,분,초로 반환
*/
select num as 학번, if(major = '컴퓨터공학과','컴공','컴공아님') as 전공 from sutdent;
...
-- 23학번 학생 조회
select * from student where left(num, 4) = 2023; -- 고정된 길이의 문자열을 추출할 때 좋다.
												 -- 왼쪽에서4자리
select lpad(1, 5, '0'); -- 숫자도 가능
						-- 1을 5자리로 만들고 비는 부분을 0으로 채워요.
...
select datediff('2023-08-03 14:00:00','2023-08-02 13:00:00'); -- 앞에서 뒤에를 뺀
select timediff('2023-08-03 14:00:00','2023-08-02 13:00:00');
