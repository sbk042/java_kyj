-- qwe123회원이 구매(환불, 반품을 제외한)한 제품명과 옵션 목록을 확인하는 쿼리
SELECT 
	OR_ME_ID AS 주문아이디,
    PR_NAME AS 제품명,
    OP_NAME AS 옵션명,
...
-- abc123회원이 등록한 배송지 정보를 조회하는 쿼리를 작성하시오.
...
-- =는 완전히 같아야되므로 쓸 수 없다. => 대신 LIKE를 써준다.
...
-- 제품 코드가 ABC001이고, 옵션명이 무선인 제품에 달린 리뷰를 조회하는 쿼리
SELECT 
	*
FROM 
	REVIEW
...
-- 제품명 옵션의 개수를 조회하는 쿼리(제품명, 옵션수 가 조회되도록)
-- 옵션정보가 없으니까 JOIN
SELECT 
	PR_NAME AS 제품명,
	COUNT(*) AS 옵션수 
