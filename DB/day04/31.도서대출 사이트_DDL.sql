...
- 대출 기간은 14일, 연체 할때마다 연체기간 지난만큼 도서 대출 불가
- 도서는 대출 가능, 대출 중, 예약 중, 예약가능 만 존재
- 같은 도서는 도서번호 마지막 숫자만 다름
- 분류번호3자리.도서번호(영문,숫자, 최대7자리) v1(1권,2권,...처럼 시리즈로 된 경우) 2(같은 도서 중 2번째 도서)
- 101.a123b v1 1

- 대출/반납은 회원만 가능
- 회원가입은 아이디, 비번, 전화번호, 생년월일를 입력하여 가입한다.
- 회원은 회원과 관리자로 구분된다.
...
CREATE TABLE `loan` (
	`lo__num`	int	NOT NULL PRIMARY KEY AUTO_INCREMENT,
