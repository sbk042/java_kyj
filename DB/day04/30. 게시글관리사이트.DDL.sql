...
트리거 : 이벤트(insert, update, delete)에 반응해 자동으로 실행되는 작업
	=> 데이터 일관성을 유지하기 위해
트리거 예시
- 쇼핑몰에서 회원이 주문을 완료하면 주문리스트 테이블에 데이터가 추가 => 옵션 수량이 변경
...
-- 게시글이 삭제됐을 때 해당 게시글을 작성한 회원의 게시글 수를 1줄이는 트리거를 작성해보세요.
...
-- 주문 테이블에 데이터가 추가된다. => 주문 리스트에 데이터가 추가 => 각 주문 리스트에 맞는 옵션에 수량이 변경된다.
...
VALUES(5, 45000,45000,0, 4500,'qwe123',2);
-- 아래 쿼리가 실행될 때 트리거가 동작되어야 한다.
INSERT INTO ORDER_LIST(OL_AMOUNT, OL_PRICE, OL_OR_NUM, OL_OP_NUM)
VALUES(1, 15000 , 3, 1),(1,30000,3,3);

-- 회원이 주문한 내역을 구매확정으로 바꾸면 포인트가 적립되는 트리거를 작성
...
BEGIN
	...
	IF NEW.OR_STATE = '구매확정' THEN -- 만약 새로바뀐 OR_STATE가 '구매확정'이면
		UPDATE MEMBER
		SET
...
-- 확인
UPDATE `ORDER` SET OR_STATE = '구매확정' WHERE OR_NUM = 5;
