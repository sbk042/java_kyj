...
-- 도서 상태를 수정 해줘야함(대출을 했으므로 대출 중이라고 바꿔줘야 한다.)
UPDATE BOOK
SET
...
-- 1.) 대출 테이블에 반납일을 수정해준다.(반납한 날짜로 수정)
UPDATE LOAN
SET
	LO_RETURN_DATE = DATE_FORMAT(NOW(),'%Y%m%d')
WHERE 
	LO_BO_NUM = (SELECT BO_NUM FROM BOOK WHERE BO_TITLE = '자바의 정석') 
AND
	LO_ME_ID = 'QWE123';

-- 2.) 도서 테이블에 해당 도서의 상태를 수정(대출 가능으로 바꿔줘야함)
UPDATE BOOK
SET
...
-- 4.) 연체일을 계산해서 회원 테이블을 수정해줘야 한다.
UPDATE MEMBER
SET
	ME_TOTAL_OVERDUE = ME_TOTAL_OVERDUE +  (SELECT 
		CASE
		-- 정상적으로 반납을 했을 때 음수가 나오므로 음수로 나올 때는 0 처리를 해줘야한다.
...
-- 회원이 대출한 모든 도서가 반납됐을 때 연체일이 있으면 다음날부터 연체 적용이 
-- 되도록 수정해줘야한다.
UPDATE MEMBER 
SET 
