...
<body>
	<h1>게시글 수정</h1>
	<form action="<c:url value='/board/update'/>" method="post" enctype="multipart/form-data"> <!-- ebctype 첨부파일을 위한 --> 
	...
	<script>
      ...
      // 첨부파일 제거 했을 때 다시 추가 할 수 있도록 만듬
      $('#summernote').summernote('code', '${board.bo_contents}');
      $('.btn-del').click(function(e){ // .btn-del을 클릭하면 
    	  e.preventDefault(); // 
    	  let fi_num = $(this).data('num'); // 첨부파일을 가져와서 추가하는 작업
    	  $(this).parent().after('<input type="hidden" name="delFiles" value="'+fi_num+'">')
    	  $(this).parent().remove(); // 클릭한 요소를 제거한다.
