...
public class Array {
	...
	public static int [] createRandomArray(int min, int max, int arr[]) {
		...
		int count = 0;
		while(count < arr.length) { //저장이 3개가 될 때 까지 반복
	 ...
	 *  매개변수 : 배열 = > int arr[]
	 *  리턴타입 : 중복여부 =>  boolean (있는지 없는지 중복 체크)
	 *  메서드명 : arrayCheck
	 */
	public static boolean arrayCheck(int arr[]) {
		...
		// arr  배열의 다른 번지들끼리 비교해서 같은 값이 있는지 확이하는 반복문
		for(int i = 0; i<arr.length; i++) {
