...
public class DBTest1 {
	...
	public static void main(String[] args) {
		...
		// Run -> Run Configuration -> common -> MS949 
		...
		try {
			...
			ps.setString(1, id); // 문자면 setString 정수면 setInt
			ps.setString(2, pw); // ?가 있는 만큼 setXXX이 나와야됨
		...
		*/
		...
		try {
			// 연결한 connection을 닫아줌
			if(con != null && !con.isClosed()) { //isClosed = 닫혀있는지
				con.close(); // 먼저 null이 아닌지 확인하고 닫혀 있지 않으면 닫아주세요. 
			}
			sc.close();
		} catch (SQLException e) {
			e.printStackTrace();
