...
public class BoardController {
	...
	private BoardService boardService = new BoardServiceImp(); //BoardService에 일을 시키기 위해
	...
	private void deleteBoard() {
		...
		if(boardService.deleteBoard(board)) {//서비스에게 일 시키기
	...
	private void updateBoard() {
		...
		if(boardService.updateBoard(board)) {//서비스에게 일 시키기
	...
	//1.) 게시글 정보를 입력(제목, 작성자아이디를 입력해야됨)
		Scanner sc = new Scanner(System.in);
		System.out.println("제목 : ");
		String title = sc.nextLine();
		System.out.println("아이디 : ");
		String id = sc.next(); //아이디는 공백이 없어야해서 next로 받는다
